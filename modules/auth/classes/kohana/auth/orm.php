...
class Kohana_Auth_ORM extends Auth {
	...
	public function _login($user, $password, $remember)
	{
		if ( ! is_object($user))
		{
			$user_type = $this->config->login_mode;
			// Load the user
			$user = ORM::factory('user', array($user_type => $user));
		}

		// If the passwords match, perform a login
		if ($user->has('roles', ORM::factory('role', array('name' => $this->config['login_role']))) AND $user->password === $password)
		{
			if ($remember === TRUE)
			{
	...
	public function password($user)
	{
		if ( ! is_object($user))
		{
			$user_type = Validate::email($user) ? 'email' : 'username';
			// Load the user
			$user = ORM::factory('user', array($user_type => $user));
		}
	...
	protected function complete_login($user)
	{
		...
		// Set the last login user agent data
		$user->last_ua = Request::$user_agent;
