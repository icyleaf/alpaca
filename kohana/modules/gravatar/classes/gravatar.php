...
class Gravatar {
	...
	public function __toString()
	{
		if ( $this->_config['render'] )
			...
			return (string) $this->generate_url();
	}
	...
	{
		/*if ( empty($email) )
			throw new Gravatar_Exception('The email address is empty!');
		else */
	...
	public function default_image($url)
	{
		if (validate::url($url))
			...
			throw new Gravatar_Exception('The url ":url" is improperly formatted', array(':url'=>$url));
	...
	public function render($view = FALSE, $email = NULL)
	{
		...
		$data = array('src' => array('src' => $this->generate_url()));
